...
spacypdfreader.parsers.pytesseract.image_to_string = image_to_string
from pdf2image import convert_from_path
defaults_of_conv = list(convert_from_path.__defaults__)
defaults_of_conv[0] = 96
convert_from_path.__defaults__ = tuple(defaults_of_conv)
convert_from_path.__defaults__[0]
...
df = df[df["data.year"] == 2023]
# df = df[df["data.year"] <= 2019]
# df = df[df["data.issue"] <= 12]
with open("df.pickle","wb") as f:
    df.to_pickle(f)
    
def map_load(doc_path="./doc_trf_2", split=False)-> map:
    """
...
if __name__ == '__main__':
    ...
    for pages, file in df[["num_pages", "file_name"]].values:
        doc = pdf_reader(folder+"\\"+file, nlp, parser, n_processes=24, lang="deu", page_range=(2, pages-1))
